...
#include "PixTestPhOptimizationPlay.hh"
#include "log.h"

using namespace std;
using namespace pxar;

ClassImp(PixTestPhOptimizationPlay)

PixTestPhOptimizationPlay::PixTestPhOptimizationPlay() {}

PixTestPhOptimizationPlay::PixTestPhOptimizationPlay( PixSetup *a, std::string name ) :  PixTest(a, name), fParNtrig(1), fSafetyMarginLow(20), fQuantMax(0.98), fVcalMax(100) {
  PixTest::init();
  init();
}

bool PixTestPhOptimizationPlay::setParameter(string parName, string sval) {
  bool found(false);
  string str1, str2;
  string::size_type s1;
  int pixc, pixr;
...
void PixTestPhOptimizationPlay::init() {
...
void PixTestPhOptimizationPlay::bookHist(string /*name*/) {}

PixTestPhOptimizationPlay::~PixTestPhOptimizationPlay() {}

void PixTestPhOptimizationPlay::doTest() {

  TStopwatch t;
  
  gStyle->SetPalette(1);
  cacheDacs();
  bigBanner(Form("PixTestPhOptimizationPlay::doTest() Ntrig = %d", fParNtrig ));
  fDirectory->cd();
  PixTest::update();
  ...
  LOG(logINFO) << "PixTestPhOptimizationPlay::doTest() done, duration: " << seconds << " seconds";
...
void PixTestPhOptimizationPlay::BlacklistPixels(std::vector<std::pair<uint8_t, pair<int, int> > > &badPixels, int aliveTrig){
...
pxar::pixel* PixTestPhOptimizationPlay::RandomPixel(std::vector<std::pair<uint8_t, pair<int, int> > > &badPixels, uint8_t iroc){
...
void PixTestPhOptimizationPlay::GetMaxPhPixel(map<int, pxar::pixel > &maxpixels,   std::vector<std::pair<uint8_t, pair<int,int> > >  &badPixels){
...
void PixTestPhOptimizationPlay::GetMinPhPixel(map<int, pxar::pixel > &minpixels, map<int, int> &minVcal, std::vector<std::pair<uint8_t, pair<int,int> > >  &badPixels){
...
void PixTestPhOptimizationPlay::DrawPhMaps(std::map<int, int> &minVcal, std::vector<std::pair<uint8_t, std::pair<int, int> > > &badPixels){
  vector<uint8_t> rocIds = fApi->_dut->getEnabledRocIDs();
  string name, title;
...
void PixTestPhOptimizationPlay::DrawPhCurves(map<int, pxar::pixel > &maxpixels, map<int, pxar::pixel > &minpixels, std::map<uint8_t, int> &po_opt, std::map<uint8_t, int> &ps_opt){
  vector<uint8_t> rocIds = fApi->_dut->getEnabledRocIDs();
  string name, title;
  TH1D *h1(0); 
  for(unsigned int roc_it = 0; roc_it < rocIds.size(); roc_it++){
...
void PixTestPhOptimizationPlay::MaxPhVsDacDac(std::vector< std::pair<uint8_t, std::pair<uint8_t, std::vector<pxar::pixel> > > > &dacdac_max, map<int, pxar::pixel> maxpixels){
...
void PixTestPhOptimizationPlay::MinPhVsDacDac(std::vector< std::pair<uint8_t, std::pair<uint8_t, std::vector<pxar::pixel> > > > &dacdac_min, map<int, pxar::pixel> minpixels, std::map<int, int> &minVcal){
...
void PixTestPhOptimizationPlay::SetMinThr(){
  fMinThr=0;
...
void PixTestPhOptimizationPlay::getTH2fromMaps(std::vector< std::pair<uint8_t, std::pair<uint8_t, std::vector<pxar::pixel> > > > &dacdac_max,   std::vector< std::pair<uint8_t, std::pair<uint8_t, std::vector<pxar::pixel> > > > &dacdac_min, std::vector<TH2D* > &th2_max, std::vector<TH2D* > &th2_min){

  TH2D* h2 = new TH2D("h2", "h2", 256, 0., 256., 256, 0., 256.);
...
void PixTestPhOptimizationPlay::optimiseOnMapsNew(std::map<uint8_t, int> &po_opt, std::map<uint8_t, int> &ps_opt,  std::vector<TH2D* > &th2_max, std::vector<TH2D* > &th2_min, std::vector<TH2D* > &th2_sol){
  vector<uint8_t> rocIds = fApi->_dut->getEnabledRocIDs(); 
  TH2D* hmin(0);
  TH2D* hmax(0);
  TH2D* hsol = new TH2D("hsol", "hsol", 256, 0., 256., 256, 0., 256.);
  for(unsigned int i=0; i< fApi->_dut->getNEnabledRocs() ; i++){
  ...
  LOG(logINFO)<<"Now scanning through th2 for solutions.";
  for(int ibinx=21;ibinx < hmin->GetNbinsX()+1; ibinx++){ //Excluding PhScale lower than 20
	 for(int ibiny=1; ibiny < hmin->GetNbinsY()+1; ibiny++){
